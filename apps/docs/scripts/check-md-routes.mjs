 ...
 *
 * Checked against the unfenced text only. These docs are largely diffs and
 * terminal output, and a fence may legitimately contain any of these — the
 * suppression pages quote `overlock-ignore` directives, and several rule pages
 * show paths that look like root-relative links. Scanning the whole document
 * flags those, and the obvious "fix" is to rewrite the contents of fences, which
 * corrupts the examples.
 */
...
export async function checkMdRoutes(distDir = DEFAULT_DIST) {
  ...
  for (const md of [...mdFiles].sort()) {
    ...
    // that catches a page linking to `./severity.md` when the page is at
    // `learn/severity` — a well-formed link that nothing else in the build reads.
    if (!prefix) continue;
