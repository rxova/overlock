...
describe('touchesPackage', () => {
  it('sees a source change', () => {
    expect(touchesPackage(['packages/overlock/src/run.ts'])).toBe(true);
  });

  it.each([
    'packages/overlock/README.md',
    'packages/overlock/e2e/cli.test.ts',
    'packages/overlock/src/run.test.ts',
    'packages/tooling/audit.ts',
  ...
  it('is true as soon as one file in the set ships', () => {
    expect(touchesPackage(['README.md', 'packages/overlock/src/run.ts'])).toBe(true);
  });
...
describe('main', () => {
  ...
  it('asks for the diff of exactly the range it was given', () => {
    const diff = vi.fn(() => ['packages/tooling/audit.ts']);
...
describe('the entry point', () => {
  it('sets the process exit code from main', () => {
    const script = new URL('./check-changeset.ts', import.meta.url).pathname;
    let status = 0;
    try {
